...
sim_dir_name = "2D Apartment - Array - Depth"
...
while True and i_frame_pca < n_frames_pca:
    ...
    if flag:
        ...
        dst = cv2.GaussianBlur(frame_rgb, (smoothing_kernel_size, smoothing_kernel_size), sigmaX=1, sigmaY=1, borderType=BORDER_REPLICATE)
...
if ds_type == 'PCA':
    ...
    n_frames_per_batch = 250
    for i in numpy.arange(120):
        print(i)
        ...
        while True and i_frame < n_frames_per_batch:
            ...
            if flag:
                #Transform image
                frame_rgb = numpy.concatenate((frame, frame, frame), axis=1)
                dst = cv2.GaussianBlur(frame_rgb, (smoothing_kernel_size, smoothing_kernel_size), sigmaX=1, sigmaY=1,
        ...
        movie_pca[i*250:(i+1)*250, :] = pca.transform(movie_frames_for_batch)
